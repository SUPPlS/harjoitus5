...
def apina_ui1(apina, apina_teksti, canvas, start_x, start_y):
    def apina1_liikkuu():
        current_x = start_x
        kilometrin_vali = 8  # 1 km = 8 pikseliä
        total_distance = 800  # apina kulkee 800 pikseliä
        ...
        for i in range(beep_count):
            ...
            # Tarkista, tuleeko apina syödyksi
            if (i + 1) % 5 == 0 and random.random() < 0.5:  # 50% mahdollisuus joka viidennellä kilometrillä
                print("Ernestin apina tulee syödyksi!")

                random_x = random.randint(350, 550)  # Rajoita X-koordinaatti
                canvas.move(apina, random_x - start_x, 260)  # Liikuta apinaa satunnaiseen X-koordinaattiin ja tippuu alaspäin
                canvas.move(apina_teksti, random_x - start_x, 260)  # Liikuta myös tekstiä
                winsound.Beep(300, 500)  # Ääni, kun apina syödään
                return  # Lopeta thread
            else:
...
def apina_ui2(apina, apina_teksti, canvas, start_x, start_y):
    def apina2_liikkuu():
        ...
        for i in range(beep_count):
            ...
            # Tarkista, tuleeko apina syödyksi
            if (i + 1) % 5 == 0 and random.random() < 0.5:  # 50% mahdollisuus joka viidennellä kilometrillä
                print("Kernestin apina tulee syödyksi!")

                random_x = random.randint(400, 550)  # Rajoita X-koordinaatti
                canvas.move(apina, random_x - start_x, -320)  # Liikuta apinaa satunnaiseen X-koordinaattiin ja tippuu alaspäin
                canvas.move(apina_teksti, random_x - start_x, -320)  # Liikuta myös tekstiä
                winsound.Beep(300, 500)  # Ääni, kun apina syödään
                return  # Lopeta thread
            else:
                canvas.move(apina, kilometrin_vali, 0)
